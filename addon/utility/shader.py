...
def logo(context, region, width, offset, r_offset):
    ...
    if addon.preference().logo_auto_offset:
        hard_ops_name = [addon for addon in context.preferences.addons.keys() if addon in ('HardOps', 'HOps', 'HOps-master')]

        if len(hard_ops_name):
            hard_ops_name = hard_ops_name[0]
            hops_logo_size = context.preferences.addons[hard_ops_name].preferences.Hops_logo_size
            x_offset += int(6 * hops_logo_size * dpi.factor())

        boxcutter_name = [addon for addon in context.preferences.addons.keys() if addon in ('BoxCutter', 'BC', 'BC-master')]

        if len(boxcutter_name):
            boxcutter_name = boxcutter_name[0]
            bc_logo_size = context.preferences.addons[boxcutter_name].preferences.BC_indicator_size
